...
class Report:
    ...
    def criptovaluta_volume_maggiore(self,dati):
        ...
        print(f"\n The cryptocurrency with the largest volume in the last 24 hours is¨ : {currency_max} with {currencies_volume}$\n")
        return currency_max, currencies_volume


    def criptovalute_worst_increase(self,dati):
        lista_valute={}
        lista_minore=[]
        for currency in dati['data']:
        ...
        for k in sorted(lista_valute.keys()):
            if len(lista_minore)<10:
                ...
                break
        print ("\nThe list of the 10 worst cryptocurrencies per percentage increment is:\n")
    ...
    def criptovalute_incremento_migliori(self, dati):
        ...
        for k in sorted(lista_valute.keys(), reverse=True):
            if len(lista_maggiore) < 10:
                ...
                break
        print("\nThe list of the top 10 cryptocurrencies per percentage increment is :\n")
    ...
    def acquisto_criptovalute_volume_maggiore (self,dati):
        ...
        print("\nThe list of cryptocurrencies whose volume in the last 24 hours was greater than 76000000$ is:\n")
        [print(x) for x in valute_volume_maggiore]
        print(f"\nThe amount of money needed to buy a unit of all these cryptocurrencies is:¨ : {prezzo}$\n")
        return prezzo
    ...
    def acquisto_criptovalute_capitalizzazione_maggiore (self,dati):
            ...
            print ("\nThe most capitalized cryptocurrencies are : \n")
            [print(x) for x in lista_top20]
            print (f"\nThe amount of money needed to buy one unit of each of the top 20 cryptocurrencies is : {prezzo_totale}$\n")
    ...
    def acquisto_criptovalute_capitalizzazione_maggiore_giorno_precedente (self,dati):
        ...
        print (f"The total price when buying a higher capitalized cryptocurrency unit on the previous day is¨ : {prezzo_totale_precedente}")
        print (f"The total price when buying a higher capitalized cryptocurrency unit today is¨ : {prezzo_totale}")
        print (f"The percentage of gain or loss is¨ : {perdita_guadagno_percentuale}")
